...
class Trainer(object):
    ...
    def generate_validation_video(self, name, source_video=None):
        ...
        cmd = f"ffmpeg -framerate {source_framerate} -i {rgb_file_path} -c:v h264 {no_audio_video_path} -y"
        os.system(cmd)
        ...
        cmd = f'ffmpeg -i {source_video} -ss {seek} -c:v h264 {ground_truth_path} -y'
        os.system(cmd)
        ...
        cmd = f'ffmpeg -i {no_audio_video_path} -i {audio_path} -c:v h264 -c:a aac {video_path} -y'
        os.system(cmd)
