...
class Trainer(object):
    ...
    def test(self, loader, save_path=None, name=None, write_image=False):
        ...
        if self.opt.aud != '':
            os.system(f'ffmpeg -i {os.path.join(save_path, f"{name}.mp4")} -i {self.opt.aud} -c:v libx264 -preset veryslow -qp 0 -strict -2 {os.path.join(save_path, f"{name}_audio.mp4")} -y')
    ...
    def generate_validation_video(self, name, source_video=None):
        ...
        cmd = f'ffmpeg -i {no_audio_video_path} -i {audio_path} -c:v libx264 -preset veryslow -qp 0 -c:a aac -strict experimental {video_path} -y'
        os.system(cmd)
